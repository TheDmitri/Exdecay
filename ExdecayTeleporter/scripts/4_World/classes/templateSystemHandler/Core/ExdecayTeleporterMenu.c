class ExdecayTeleporterMenu extends UIScriptedMenu
{
  ...
  void SpawnButtonHandler()
  {
    ...
    GetRPCManager().SendRPC("ExdecayTeleporter", "GetExdecayTeleporterRequest",  new Param2<string,vector>(GetClientMaps().Get(selectedMap).MapName,GetClientMaps().Get(selectedMap).Positions.Get(selectedRow).PointPosition), true, NULL);
    OnHide();
  }
