...
class GLSJsonLoader<Class T>
{
	...
	static string ObjectToString(T data)
	{
		...
		if (!m_Serializer.WriteToString(data, true, string_data)) {
			GetGLSLogger().LogError("GLSJsonLoader::ObjectToString Could not stringify "+ data.ToString());
			return string.Empty;
		}
	...
	static void SaveToFile(string path, T data)
	{
		...
		if (!fh) {
			GetGLSLogger().LogError("GLSJsonLoader::SaveToFile File could not be created at "+ path);
			return;
		}
		...
		GetGLSLogger().LogError("GLSJsonLoader::SaveToFile Complete: " + path);
		CloseFile(fh);
	}
	...
	{
		if (!FileExist(path)) {
			GetGLSLogger().LogError("GLSJsonLoader::LoadFromFile File not found "+ path);
			return;
		}
		...
		if (!fh) {
			GetGLSLogger().LogError("GLSJsonLoader::LoadFromFile File could not be opened " + path);
			return;
		}
		...
		if (error != string.Empty || !success) {
			GetGLSLogger().LogError("GLSJsonLoader::LoadFromFile ERROR Parsing "+ path + error);
			return;
		}

		GetGLSLogger().LogInfo("GLSJsonLoader::LoadFromFile Loaded file: " + path);
		CloseFile(fh);
	}
};
